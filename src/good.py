...
class model_opt():
    ...
    def genToDemand(self):
        ...
        export_indices = [(r,o) for r in self.model.r for o in self.model.o if (r,o) in self.model.c_transCap]
        export_term = pyomo.quicksum(self.model.x_trans[r,o,t] * self.model.c_transLoss
                    for r in self.model.r 
                    for o in self.model.o 
                    for t in self.model.t
                    for (r,o) in export_indices)

        import_indices = [(o,r) for o in self.model.o for r in self.modlel.r if (o,r) in self.model.c_transCap]
        import_term = pyomo.quicksum(self.model.x_trans[o,r,t] * self.model.c_transLoss
                    for r in self.model.o 
                    for o in self.model.r 
                    for t in self.model.t 
                    for (o,r) in import_indices)
